...
if (isset($_POST['category_id']) && !empty($_POST['category_id']) && isset($_POST['new_name']) && !empty($_POST['new_name'])) {
    ...
    if(!$stmt->execute() || $stmt->affected_rows === 0){
        echo "Beim Updaten der Kategorie ist ein Fehler passiert";
    }else{
        echo "Name der Kategorie wurde erfolgreich angepasst!";
    }
...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    ...
    ist redundant, aber sagt aus, dass nur maximal Eintrag geholt werden soll. */
    $sql = "SELECT * FROM category WHERE id = ? LIMIT 1";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("i", $_GET['id']);
    if(!$stmt->execute()){
        echo "Konnte Kategorie mit ID ".$_GET['id']." nicht finden";
    }else{
        $res = $stmt->get_result();
        if($res->num_rows === 0){
